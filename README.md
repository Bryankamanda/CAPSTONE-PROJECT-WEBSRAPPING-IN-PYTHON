...
A web scraper for collecting used car listings from [SBT Japan](https://www.sbtjapan.com), one of Japan's largest used car exporters. Built and debugged interactively, **aided by Claude AI (Anthropic)**.
...
| Field | Description |
|---|---|
| `stock_id` | Unique SBT stock identifier |
...
Run the notebook cells sequentially. The scraper uses `subprocess` to run Playwright in a completely separate process, bypassing Jupyter's asyncio event loop conflict on Windows.
...
The batch scraper runs all 2,870 pages in batches of 25:
...
Estimated runtime: **8–10 hours** for all pages.  
Ensure your machine is set to **never sleep** before leaving it overnight.
...
This project was built interactively with assistance from **[Claude](https://claude.ai) by Anthropic**.
...
Please respect [SBT Japan's Terms of Service](https://www.sbtjapan.com) and do not hammer their servers with rapid requests.
